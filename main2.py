                ...
                slot = self.db["slots"].find_one_and_update(
                    {
                        ...
                        "occupied": False,
                        "slot_size": {"$gt": 1},
                        # Ensure we're updating the same slot
                        "_id": slot["_id"]
                    },
                    {
                        "$set": {
...
class TrendAnalyzer:
    ...
    def can_open_new_position(self, algo_type, interval, coin_pair):
        try:
            if self.db_manager.db["trading_opportunities"].find_one({"coin_pair": coin_pair, "interval": interval, "status": "OPEN", "algo_type": algo_type}):
                ...
                latest_trading_opportunity_id = None  # Adjust as per your logic if available
            ...
            for i, conditions in enumerate(conditions_list, start=1):
                ...
                if all(conditions.values()):
                    ...
                    if liquidation_price is None:
                        logging.warning(
                            f"Liquidity price is None for {coin_pair}, skipping opportunity.")
                        continue
        ...
        try:
            ...
            for position in open_positions:
                ...
                if liq_price is None:
                    logging.warning(
                        f"Liquidity price is None for ID {id}, skipping operation.")
                    continue
    ...
    def print_and_save_pnl(self):
        try:
            # Print and save total PnL for each condition per coin
            pipeline_conditions_per_coin = [
                {
                    "$group": {
                        "_id": {"algo_type": "$algo_type", "coin_pair": "$coin_pair"},
            ...
            self.save_to_file(
                "Total PnL for each condition per coin:", 'pnl_report.txt')
            for result in results_conditions_per_coin:
                ...
                message = (f"Algorithm: {algo_type}, Coin: {coin_pair}, Total PnL: {total_pnl}, "
                           f"Total Gains: {total_gains}, Total Losses: {total_losses}")
                logging.info(message)
                self.save_to_file(message, 'pnl_report.txt')

            # Print and save total PnL for each condition across all coins
            pipeline_conditions = [
                {
                    "$group": {
                        "_id": "$algo_type",
            ...
            self.save_to_file(
                "Total PnL for each condition across all coins:", 'pnl_report.txt')
            for result in results_conditions:
                algo_type = result["_id"]
                total_pnl = result["total_pnl"]
                total_gains = result.get("total_gains", 0)
                total_losses = result.get("total_losses", 0)
                message = (f"Algorithm: {algo_type}, Total PnL: {total_pnl}, "
                           f"Total Gains: {total_gains}, Total Losses: {total_losses}")
                logging.info(message)
                self.save_to_file(message, 'pnl_report.txt')

            # Print and save total PnL for each coin across all conditions
            pipeline_coins = [
                {
                    "$group": {
                        "_id": "$coin_pair",
            ...
            self.save_to_file(
                "Total PnL for each coin across all conditions:", 'pnl_report.txt')
            for result in results_coins:
                coin_pair = result["_id"]
                total_pnl = result["total_pnl"]
                total_gains = result.get("total_gains", 0)
                total_losses = result.get("total_losses", 0)
                message = (f"Coin: {coin_pair}, Total PnL: {total_pnl}, "
                           f"Total Gains: {total_gains}, Total Losses: {total_losses}")
                logging.info(message)
                self.save_to_file(message, 'pnl_report.txt')

            # Print and save total PnL across all conditions and coins
            pipeline_total = [
                {
                    "$group": {
                        "_id": None,
            ...
            if result_total:
                ...
                message = (f"Total PnL across all conditions and coins: {total_pnl}, "
                           f"Total Gains: {total_gains}, Total Losses: {total_losses}")
                logging.info(message)
                self.save_to_file(message, 'pnl_report.txt')

        except Exception as e:
            logging.error(f"Error in print_and_save_pnl: {e}")
...
def process_data(db_manager, trend_analyzer, coin_pairs, interval):
    ...
    logging.info("Fetched timestamps. Number of timestamps: %d",
                 len(timestamps))

    def process_timestamp(timestamp):
        current_rows = list(db_manager.db['combined_kline_data'].find(
            {'close_time': timestamp}))
        prev_rows = list(db_manager.db['combined_kline_data'].find({'close_time': {
            '$lt': timestamp}}).sort('close_time', pymongo.DESCENDING).limit(len(current_rows)))
        two_prev_rows = list(db_manager.db['combined_kline_data'].find({'close_time': {
            '$lt': timestamp}}).sort('close_time', pymongo.DESCENDING).limit(len(current_rows) * 2))

        for coin in set(row['coin'] for row in current_rows):
            current_row = next(
                (row for row in current_rows if row['coin'] == coin), None)
            prev_row = next(
                (row for row in prev_rows if row['coin'] == coin), None)
            two_prev_row = next(
                (row for row in two_prev_rows if row['coin'] == coin), None)

            if current_row and prev_row and two_prev_row:
                trend_analyzer.check_for_short(
                    coin, interval, current_row, prev_row, two_prev_row)
                trend_analyzer.check_for_long(
                    coin, interval, current_row, prev_row, two_prev_row)
                trend_analyzer.monitor_and_close_positions(
                    interval, current_rows, prev_rows)

    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=20) as executor:
        executor.map(process_timestamp, timestamps)

    trend_analyzer.print_and_save_pnl()


def main():
    b_time = "1m"
    backtest_name = "backtesting_" + b_time  + "main2"

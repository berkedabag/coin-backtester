...
def process_data(db_manager, trend_analyzer, coin_pairs, interval):
    ...
    def process_coin(coin, interval, current_row, prev_row, two_prev_row):
        try:
            ...
            logging.error(f"Error processing coin {coin} for interval {interval}: {e}")
        ...
        for timestamp in timestamps:
            ...
            for coin in set(row['coin'] for row in current_rows):
                ...
                if current_row and prev_row and two_prev_row:
                    futures.append(executor.submit(process_coin, coin, interval, current_row, prev_row, two_prev_row))

        for future in as_completed(futures):
            try:
                future.result()  # Retrieve results and raise exceptions if any
            except Exception as e:
                logging.error(f"Error in processing future: {e}")

    # Monitor and close positions after processing all coins
    for timestamp in timestamps:
        current_rows = list(db_manager.db['combined_kline_data'].find({'close_time': timestamp}))
        prev_rows = list(db_manager.db['combined_kline_data'].find({'close_time': {'$lt': timestamp}})
                         .sort('close_time', pymongo.DESCENDING).limit(len(current_rows)))
    ...
    trend_analyzer.print_total_pnl()



def main():
    b_time = "4h"
    backtest_name = "backtesting_" + b_time + "main1-4h"
